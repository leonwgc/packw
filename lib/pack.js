...
var runWebpack = function runWebpack(config, openFile, isDev, port, _devServer, callback) {
  if (openFile === void 0) {
    openFile = 'index';
  }
  ...
  if (isDev) {
    ...
    devServer.listen(p_1, serverConfig.host, function (err) {
      ...
      var serveUrl = "http://localhost:" + p_1 + "/" + (openFile === 'index' ? '' : openFile + '.html');
      console.log(chalk_1.default.green("dev server: " + serveUrl));
    });
  } else {
...
exports.pack = pack; // node called api

function nodeApi(isDev, config, callback) {
  if (callback === void 0) {
...
exports.default = nodeApi; //#endregion
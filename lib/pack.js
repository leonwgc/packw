...
var runWebpack = function runWebpack(config, openFile, isDev, devPort, devServerConfig, callback) {
  ...
  if (isDev) {
    ...
    devServer.listen(port_1, serverConfig.host, function (err) {
      ...
      var page = "".concat(openFile === 'index' ? '' : openFile + '.html');
      var serveUrl = "http://localhost:".concat(port_1, "/").concat(page);
      var serverUrlIp = "http://".concat(address_1.default.ip(), ":").concat(port_1, "/").concat(page);
      console.log();
      console.log(chalk_1.default.cyan('dev server running at:'));
      console.log();
      console.log('> Local:', chalk_1.default.green("".concat(serveUrl)));
      console.log();
      console.log('> Network:', chalk_1.default.green("".concat(serverUrlIp)));
    });
  } else {
...
var run = function run(dir, publicPath, isDev, port) {
  ...
  var s = glob_1.default.sync("./src/".concat(dir, "/index{.jsx,.js,.ts,.tsx}"));
  var isDir = true;
  ...
  if (!s.length) {
    ...
    if (!s.length) {
      exit("\u5165\u53E3\u6587\u4EF6\u672A\u627E\u5230 : ".concat(getProjectPath('./src/index')));
    }

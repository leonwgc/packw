...
exports.pack = (isDev = true, port = 9000, publicPath = '/') => {
  const configFile = helper.getProjectPath('./packx.entry.js');
  if (!fs.existsSync(configFile)) {
    exit(`入口配置文件不存在:packx.entry.js`);
  }

  const entry = require(configFile);
  if (typeof entry === 'object' && entry) {
    const keys = Object.keys(entry);
    if (keys.length) {
      const config = getConfig(isDev, entry, publicPath);
      return runWebpack(config, keys[0], isDev, port);
    }
  }
  exit('1');
};
